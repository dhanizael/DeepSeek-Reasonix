...
| 2 | **Verification harness** (`internal/harness`) auto-detect `go test` / `npm test` / etc. after mutating tools | **Idle** (hook in `observeAfterMutation`, never booted) | Goal/delivery **verification evidence** (model must run checks; not host auto-runner) | **KEEP** | Genuinely different: host-side automatic feedback loop. Upstream trusts the model + readiness gates. Do **not** replace upstream evidence system—add as optional post-mutation feedback. |
| 3 | **3-strike backtrack + file rollback** (`BacktrackGuard`) | **Idle** (same hook; needs harness + setter) | `repeatFailureGuard` (block identical failing tool calls); full **checkpoint rewind** | **MERGE** | Strikes-as-policy can stay, but **rollback must use** `MutationObserver` / checkpoint store / `PrepareRewind`—not ad-hoc `git checkout`. Don't fight `repeatFailureGuard`; compose (e.g. only after harness fail). |
